...
env_factors <- c("catch_area","bio01","HFP_Lights2009","V_lc1", "autocor",
                 "Primer","BarcodeRegion","BarcodeLength","SamVolume")
bio_factors <- intersect(all_BioDiv_para, colnames(BD))

mn <- c('Primer:basic*temp+human_add', 'Barcode:basic*temp+human_add' )
...
for (j in 1:RStimes) {
  ...
  for (i in 1:length(bio_factors)) {
    ...
    ##models
    #--
    b_full1 <- glmmTMB(y ~ catch_area * autocor + catch_area * bio01 + catch_area * HFP_Lights2009 + V_lc1 + SamVolume + (1|Primer), data = dt,family = beta_family())
    
    b_full2 <- glmmTMB(y ~ catch_area * autocor + catch_area * bio01 + catch_area * HFP_Lights2009 + V_lc1 + SamVolume + (1|BarcodeRegion), data = dt,family = beta_family())
    
    ##
    models_add <- list(b_full1, b_full2 )
    names(models_add) <- mn
