...
env_factors <- c("catch_area","bio01","HFP_Lights2009",
...
for (i in 1:length(bio_factors)) {
    ...
    ##
    models_add <- list(b_null1,b_basic1,b_basic1_minus,b_temp1,b_temp1_minus,b_full1,b_full1_minus,
                       b_null2,b_basic2,b_basic2_minus,b_temp2,b_temp2_minus,b_full2,b_full2_minus )
    names(models_add) <- mn
    ...
    tmp <- list(as.data.frame(anova(b_null1, b_basic1_minus)), #test plus 'catch_area'
                ...
                as.data.frame(anova(b_temp2_minus, b_basic2)), # test 'bio02'
                as.data.frame(anova(b_temp2, b_temp2_minus)), # test 'catch_area:bio02'
    ...
    anova_test_add <- data.frame(
                             BD = bio_factors[i],
                             random = rep(c('Primer','Barcode'), each = 6),
                             test_term = rep(c('catch_area','catch_area:autocor','bio01','catch_area:bio01','human','catch_area:human'),2),
