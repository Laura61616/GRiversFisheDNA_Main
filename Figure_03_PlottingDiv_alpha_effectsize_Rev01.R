...
data <- subset(data, BD %in% all_BioDiv_para & model2 == 'basic*temp+human_add')
...
factors <- c("catch_area","bio01","HFP_Lights2009","V_lc1","catch_area:bio01","catch_area:HFP_Lights2009","catch_area:autocor","autocor")
fac_names <- c("Catchment Area", "Mean Annual Temp.", "Nighttime Lights", "%Cropland", "Catchment Area\n×Mean Annual Temp.", "Catchment Area\n×Nighttime Lights", "Catchment Area\n×Spatial autocor", "Spatial autocor")
bio_factors <- all_BioDiv_para

data <- subset(res, model2 == "basic*temp+human_add" & BD %in% bio_factors & factor %in% factors)
data <- subset(data, (BD2 %in% c('SR','Redundancy') & random == 'Primer')|(BD2 %in% c('MNTD','GD') & random == 'Barcode'))
